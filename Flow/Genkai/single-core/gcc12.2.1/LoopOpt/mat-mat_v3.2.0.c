/* mat-mat_v3.2.0.c - AVX-512 8x16 micro-kernel */
#include <stdio.h>
#include <stdlib.h>
#include <sys/time.h>
#include <immintrin.h>
...
static inline void micro_kernel_8x16_avx512(int K, double *A, double *B, double *C, int ldc) {
            ...
            for(int jj=0; jj<N; jj+=BLOCK_J) {
                ...
                for(int i=0; i<i_size; i+=MR) {
                    ...
                    for(int j=0; j<j_size; j+=NR) {
                        ...
                        if(mr==MR && nr==NR) micro_kernel_8x16_avx512(k_size, A_ptr, B_ptr, C_ptr, N);
