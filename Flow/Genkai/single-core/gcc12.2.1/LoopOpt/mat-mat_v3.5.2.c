/* mat-mat_v3.5.2.c - AVX-512 8x16 + BLOCK_K=1024 + BLOCK_J=128 + prefetch */
/* FINAL SOTA: 39.85 GFLOPS (49.8% efficiency) */
#include <stdio.h>
#include <stdlib.h>
#include <sys/time.h>
#include <immintrin.h>
...
static inline void pack_A(int M, int K, double *A, int lda, double *packed) {
    for(int i=0; i<M; i+=MR) for(int k=0; k<K; k++) {
        for(int ir=0; ir<MR && i+ir<M; ir++) packed[i*K + k*MR + ir] = A[(i+ir)*lda + k];
        for(int ir=(i+MR<=M)?MR:(M-i); ir<MR; ir++) packed[i*K + k*MR + ir] = 0.0;
    }
}
static inline void pack_B(int K, int N, double *B, int ldb, double *packed) {
    for(int j=0; j<N; j+=NR) for(int k=0; k<K; k++) {
        for(int jr=0; jr<NR && j+jr<N; jr++) packed[(j/NR)*K*NR + k*NR + jr] = B[k*ldb + j + jr];
        for(int jr=(j+NR<=N)?NR:(N-j); jr<NR; jr++) packed[(j/NR)*K*NR + k*NR + jr] = 0.0;
    }
}
static inline void micro_kernel_8x16_avx512_pf(int K, double *A, double *B, double *C, int ldc) {
    ...
    for(int k=0; k<K; k++) {
        if((k&7)==0 && k+16<K) {
            __builtin_prefetch(&A[(k+16)*MR], 0, 3);
            __builtin_prefetch(&B[(k+16)*NR], 0, 3);
        }
        __m512d b0 = _mm512_load_pd(&B[k*NR]);
        __m512d b1 = _mm512_load_pd(&B[k*NR+8]);
        __m512d a0=_mm512_set1_pd(A[k*MR+0]); c00=_mm512_fmadd_pd(a0,b0,c00); c01=_mm512_fmadd_pd(a0,b1,c01);
        __m512d a1=_mm512_set1_pd(A[k*MR+1]); c10=_mm512_fmadd_pd(a1,b0,c10); c11=_mm512_fmadd_pd(a1,b1,c11);
        __m512d a2=_mm512_set1_pd(A[k*MR+2]); c20=_mm512_fmadd_pd(a2,b0,c20); c21=_mm512_fmadd_pd(a2,b1,c21);
        __m512d a3=_mm512_set1_pd(A[k*MR+3]); c30=_mm512_fmadd_pd(a3,b0,c30); c31=_mm512_fmadd_pd(a3,b1,c31);
        __m512d a4=_mm512_set1_pd(A[k*MR+4]); c40=_mm512_fmadd_pd(a4,b0,c40); c41=_mm512_fmadd_pd(a4,b1,c41);
        __m512d a5=_mm512_set1_pd(A[k*MR+5]); c50=_mm512_fmadd_pd(a5,b0,c50); c51=_mm512_fmadd_pd(a5,b1,c51);
        __m512d a6=_mm512_set1_pd(A[k*MR+6]); c60=_mm512_fmadd_pd(a6,b0,c60); c61=_mm512_fmadd_pd(a6,b1,c61);
        __m512d a7=_mm512_set1_pd(A[k*MR+7]); c70=_mm512_fmadd_pd(a7,b0,c70); c71=_mm512_fmadd_pd(a7,b1,c71);
    }
    __m512d t;
    ...
    t=_mm512_loadu_pd(&C[4*ldc]); _mm512_storeu_pd(&C[4*ldc], _mm512_add_pd(c40,t));
    t=_mm512_loadu_pd(&C[4*ldc+8]); _mm512_storeu_pd(&C[4*ldc+8], _mm512_add_pd(c41,t));
    t=_mm512_loadu_pd(&C[5*ldc]); _mm512_storeu_pd(&C[5*ldc], _mm512_add_pd(c50,t));
    t=_mm512_loadu_pd(&C[5*ldc+8]); _mm512_storeu_pd(&C[5*ldc+8], _mm512_add_pd(c51,t));
    t=_mm512_loadu_pd(&C[6*ldc]); _mm512_storeu_pd(&C[6*ldc], _mm512_add_pd(c60,t));
    t=_mm512_loadu_pd(&C[6*ldc+8]); _mm512_storeu_pd(&C[6*ldc+8], _mm512_add_pd(c61,t));
    t=_mm512_loadu_pd(&C[7*ldc]); _mm512_storeu_pd(&C[7*ldc], _mm512_add_pd(c70,t));
    t=_mm512_loadu_pd(&C[7*ldc+8]); _mm512_storeu_pd(&C[7*ldc+8], _mm512_add_pd(c71,t));
}
static inline void micro_kernel_scalar(int K, int mr, int nr, double *A, double *B, double *C, int ldc) {
    for(int k=0; k<K; k++) for(int i=0; i<mr; i++) {
            ...
            for(int jj=0; jj<N; jj+=BLOCK_J) {
                ...
                for(int i=0; i<i_size; i+=MR) {
                    ...
                    for(int j=0; j<j_size; j+=NR) {
                        ...
                        if(mr==MR && nr==NR) micro_kernel_8x16_avx512_pf(k_size, A_ptr, B_ptr, C_ptr, N);
                        else micro_kernel_scalar(k_size, mr, nr, A_ptr, B_ptr, C_ptr, N);
                    }
                }
            }
        }
...
int main(int argc, char **argv) {
    ...
    printf("N=%d AVX-512 8x16 BLOCK_K=1024 BLOCK_J=128\n", N);
    ...
    for(int i=0; i<N; i++) for(int j=0; j<N; j++) { a[i*N+j]=(double)(i+1)+(double)(j+1)*0.01; b[i*N+j]=(double)(i+1)+(double)(j+1)*0.01; c[i*N+j]=0.0; }
