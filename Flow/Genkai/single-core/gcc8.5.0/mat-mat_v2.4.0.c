/* mat-mat_v2.4.0.c - AVX-512 4x16 with k-loop unrolling (2x) */
#include <stdio.h>
#include <stdlib.h>
#include <sys/time.h>
#include <immintrin.h>
...
// 4x16 AVX-512 micro-kernel with k-loop 2x unrolling
static inline void micro_kernel_4x16_unroll2(int K, double *A, double *B, double *C, int ldc)
{
    ...
    for(; k < K-1; k += 2){
        // Iteration k
        __m512d b0_0 = _mm512_load_pd(&B[k*NR]);
        __m512d b1_0 = _mm512_load_pd(&B[k*NR+8]);
        __m512d a0_0 = _mm512_set1_pd(A[k*MR+0]);
        __m512d a1_0 = _mm512_set1_pd(A[k*MR+1]);
        __m512d a2_0 = _mm512_set1_pd(A[k*MR+2]);
        __m512d a3_0 = _mm512_set1_pd(A[k*MR+3]);

        c00 = _mm512_fmadd_pd(a0_0, b0_0, c00); c01 = _mm512_fmadd_pd(a0_0, b1_0, c01);
        c10 = _mm512_fmadd_pd(a1_0, b0_0, c10); c11 = _mm512_fmadd_pd(a1_0, b1_0, c11);
        c20 = _mm512_fmadd_pd(a2_0, b0_0, c20); c21 = _mm512_fmadd_pd(a2_0, b1_0, c21);
        c30 = _mm512_fmadd_pd(a3_0, b0_0, c30); c31 = _mm512_fmadd_pd(a3_0, b1_0, c31);

        // Iteration k+1
        __m512d b0_1 = _mm512_load_pd(&B[(k+1)*NR]);
        __m512d b1_1 = _mm512_load_pd(&B[(k+1)*NR+8]);
        __m512d a0_1 = _mm512_set1_pd(A[(k+1)*MR+0]);
        __m512d a1_1 = _mm512_set1_pd(A[(k+1)*MR+1]);
        __m512d a2_1 = _mm512_set1_pd(A[(k+1)*MR+2]);
        __m512d a3_1 = _mm512_set1_pd(A[(k+1)*MR+3]);

        c00 = _mm512_fmadd_pd(a0_1, b0_1, c00); c01 = _mm512_fmadd_pd(a0_1, b1_1, c01);
        c10 = _mm512_fmadd_pd(a1_1, b0_1, c10); c11 = _mm512_fmadd_pd(a1_1, b1_1, c11);
        c20 = _mm512_fmadd_pd(a2_1, b0_1, c20); c21 = _mm512_fmadd_pd(a2_1, b1_1, c21);
        c30 = _mm512_fmadd_pd(a3_1, b0_1, c30); c31 = _mm512_fmadd_pd(a3_1, b1_1, c31);
    }
            ...
            for(int jj=0; jj<N; jj+=BLOCK_J){
                ...
                for(int i=0; i<i_size; i+=MR){
                    ...
                    for(int j=0; j<j_size; j+=NR){
                        ...
                        if(mr == MR && nr == NR){
                            micro_kernel_4x16_unroll2(k_size, A_ptr, B_ptr, C_ptr, N);
                        } else {
...
int main(int argc, char **argv)
{
    ...
    printf("N=%d AVX-512 4x16 k-unroll2\n", N);
