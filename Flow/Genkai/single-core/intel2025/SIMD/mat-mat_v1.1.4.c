...
// 4x24レジスタブロッキング + k-loop 8段アンローリング
static inline void micro_kernel_4x24_unroll8(int K, const double* __restrict__ A, int lda,
                                              ...
                                              double* __restrict__ C, int ldc)
{
    ...
    for (k = 0; k + 8 <= K; k += 8) {
        #define UNROLL_K(kk) \
        { \
            __m512d b0 = _mm512_loadu_pd(&B[(k+kk) * ldb + 0]); \
            __m512d b1 = _mm512_loadu_pd(&B[(k+kk) * ldb + 8]); \
            __m512d b2 = _mm512_loadu_pd(&B[(k+kk) * ldb + 16]); \
            __m512d a0 = _mm512_set1_pd(A[0 * lda + k + kk]); \
            __m512d a1 = _mm512_set1_pd(A[1 * lda + k + kk]); \
            __m512d a2 = _mm512_set1_pd(A[2 * lda + k + kk]); \
            __m512d a3 = _mm512_set1_pd(A[3 * lda + k + kk]); \
        ...
        UNROLL_K(0); UNROLL_K(1); UNROLL_K(2); UNROLL_K(3);
        UNROLL_K(4); UNROLL_K(5); UNROLL_K(6); UNROLL_K(7);
        #undef UNROLL_K
    }
            ...
            for (int ii = 0; ii < N; ii += BLOCK_I) {
                ...
                for (i = ii; i + 4 <= i_end; i += 4) {
                    for (j = jj; j + 24 <= j_end; j += 24) {
                        micro_kernel_4x24_unroll8(K_block, &a[i * N + kk], N, &b[kk * N + j], N, &c[i * N + j], N);
                    }
                    for (; j < j_end; j++) {
...
int main(int argc, char **argv)
{
    ...
    if (out == 1) {
        printf("result:\n");
        for (i = 0; i < N; i++) { for (j = 0; j < N; j++) printf(" %8.2f", c[i * N + j]); printf("\n"); }
    }
