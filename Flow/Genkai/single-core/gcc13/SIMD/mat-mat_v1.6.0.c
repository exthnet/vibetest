/*
 * mat-mat_v1.6.0.c - Matrix Multiplication with Tuned Parameters
 * PG1.3 - GCC 13.3.1 SIMD Optimization
 *
 * v1.6.0: Based on v1.3.0 (6x16 kernel), tuned for GCC 13.3.1
 *         + Larger cache blocks for L2 (2MB)
 *         + 2-level loop unrolling in micro-kernel
...
/* Cache blocking - tuned for 2MB L2 */
#define MC 96
#define KC 512
#define NC 512
...
/* 6x16 micro-kernel with loop unrolling by 2 */
static inline void micro_kernel_6x16_unroll2(int K, const double *A, int lda,
                                              ...
                                              double *C, int ldc)
{
    ...
    for (; k + 1 < K; k += 2) {
        ...
        _mm_prefetch((const char*)&B[(k+4) * ldb + 8], _MM_HINT_T0);
        ...
        /* Iteration 2 */
        b0 = _mm512_loadu_pd(&B[(k+1) * ldb]);
        b1 = _mm512_loadu_pd(&B[(k+1) * ldb + 8]);

        a0 = _mm512_set1_pd(A[0 * lda + k + 1]);
        a1 = _mm512_set1_pd(A[1 * lda + k + 1]);
        a2 = _mm512_set1_pd(A[2 * lda + k + 1]);
        a3 = _mm512_set1_pd(A[3 * lda + k + 1]);
        a4 = _mm512_set1_pd(A[4 * lda + k + 1]);
        a5 = _mm512_set1_pd(A[5 * lda + k + 1]);
            ...
            for (ii = 0; ii < N; ii += MC) {
                ...
                for (i = ii; i + MR <= i_end; i += MR) {
                    for (j = jj; j + NR <= j_end; j += NR) {
                        micro_kernel_6x16_unroll2(k_len,
                                                 &a[i * N + kk], N,
                                                 &b[kk * N + j], N,
                                                 &c[i * N + j], N);
                    }
